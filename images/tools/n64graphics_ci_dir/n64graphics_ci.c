...
ngci_rgba *rawci2rgba(const uint8_t *rawci, const uint8_t *palette, int width, int height, int depth)
{
    ...
    img = raw2rgba_ngci(raw_rgba, width, height, 16);
...
#ifdef N64GRAPHICS_CI_STANDALONE
...
static void print_version(void)
{
    ERROR("n64graphics v" N64GRAPHICS_VERSION ", using:\n"
        "  %s\n"
        "  %s\n",
        n64graphics_ci_get_read_version(), n64graphics_ci_get_write_version());
}
...
int main(int argc, char* argv[])
{
    ...
    if (config.mode == MODE_IMPORT) {
        ...
        case IMG_FORMAT_CI:
            imgr = png2rgba_ngci(config.img_filename, &config.width, &config.height);
        ...
        case IMG_FORMAT_CI:
            ...
            res = rgba2png_ngci(config.img_filename, imgr, config.width, config.height);
...
#endif // N64GRAPHICS_CI_STANDALONE
