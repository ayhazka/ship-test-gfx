...
DM64Globals::DM64Globals() {
	...
	context = LUS::Context::CreateInstance("Simple test rendering", "simple", OTRFiles);
}
...
{
    DM64Globals::Instance->context->MainLoop(run_one_game_iter);
}
...
{
    return (Gfx*)GetResourceDataByName(path, false);
}

extern "C" char* LoadTexByName(const char* path)
{
    return (char*)GetResourceDataByName(path, false);
}
...
extern "C" uint32_t GetCurrentWidth() {
    return DM64Globals::Instance->context->GetCurrentWidth();
}

extern "C" uint32_t GetCurrentHeight() {
    return DM64Globals::Instance->context->GetCurrentHeight();
}
