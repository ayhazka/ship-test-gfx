...
void aPoleFilterImpl(uint8_t flags, int16_t gain, POLEF_STATE state) {
    int16_t *h1 = rspa.adpcm_table[0][0], *h2 = rspa.adpcm_table[0][1];
    int16_t l1, l2, h2_before[8];
    int16_t *in = rspa.buf.as_s16 + rspa.in / sizeof(int16_t);
    int16_t *out = rspa.buf.as_s16 + rspa.out / sizeof(int16_t);
    int nbytes = ROUND_UP_16(rspa.nbytes);
    int i, j;
    ...
    do {
        ...
        for (i = 0; i < 8; i++) {
            int32_t acc = 0;
            const int16_t *x = h2, *y = &frame[i];

            for (j = i; j > 0; j--)
                acc += *(x++) * *(--y);

            acc += h1[i] * l1 + h2_before[i] * l2 + frame[i] * gain;
            out[i] = clamp16(acc >> 14);
        }
    ...
    memcpy(state, out - 4, sizeof(POLEF_STATE));
}
